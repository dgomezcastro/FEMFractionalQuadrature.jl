...
for (k, s) in enumerate(ss)
    filename = "figs/f=1_convergence_rho_1d_s_$(s).jld2"
    dict = load(filename)
    ρs = dict["ρs"]
    errsΡs = dict["errors"]
    ...
    elseif s < 0.5
        ...
        c = sum(cs) / length(cs)
        ρs_extended = sort(ρs)
        ρs_extended = [0.8 * minimum(ρs); ρs_extended; 1.2 * maximum(ρs)]
...
savefig(plt, "figs/f=1_convergencerho_1d.pdf")
