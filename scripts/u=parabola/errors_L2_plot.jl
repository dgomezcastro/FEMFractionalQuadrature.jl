...
for (k, s) in enumerate(ss)
    ...
    cs = (errsHs) ./ (hs .^ (3 / 2))
    c = sum(cs) / length(cs)
    plot!(plt, hs_extended, c * hs_extended .^ (exp), label="", linestyle=:dash, color=COLORS[k])
end

xlabel!(plt, L"h")
ylabel!(plt, L"\lVert u_h - u^\star \rVert_{L^2(\Omega)}")
savefig(plt, "figs/u=parabola_ConvWFEM_L2_compare.pdf")