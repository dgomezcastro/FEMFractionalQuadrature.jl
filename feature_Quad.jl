...
export generate_quadrature, generate_optimal_quadrature, generate_optimal_quadrature_WFEM, find_element, quadrature_element_locations, weights_1d
using LinearAlgebra, Base.Threads, ToeplitzMatrices
...
function generate_optimal_quadrature_WFEM_mod(ρ::Float64, s::Float64)
    "Function generating optimal cube superset of domain (a,b) given value of rho, for the WFEM case"
    "Returns array of baricenters of different cubes of uniform size ρ^d"
...
function weights_1d(quadrature::Vector{Float64}, len_quadrature::Int, s::Float64)

    weights = zeros(len_quadrature, len_quadrature)
    @threads for k in 1:len_quadrature
        for l in k+1:len_quadrature
            weights[k, l] = abs(quadrature[k]-quadrature[l])^(-1-2*s)
        end
    end
    weights = triu(weights) + triu(weights, 1)'
    return weights
end

function weights_1d_toepz(quadrature::Vector{Float64}, len_quadrature::Int, s::Float64)

    weights_row = zeros(len_quadrature)
    weights_column = zeros(len_quadrature)
    @threads for k in 2:len_quadrature
        weights_row[k] = abs(quadrature[1]-quadrature[k])^(-1-2*s)
        weights_column[k] = abs(quadrature[k]-quadrature[1])^(-1-2*s)
    end
    return Toeplitz(weights_column, weights_row)
end


function sum_weights_onedirection_fast(quadrature::Vector{Float64}, len_quadrature::Int, s::Float64)
    W_func(x) = abs(x) == 0 ? 0 : 1 / abs(x)^(1+2*s)
    WW = zeros(len_quadrature)
    @threads for k in range(1, len_quadrature)
        WW[k] = sum(W_func(quadrature[k]-quadrature[l]) for l in 1:len_quadrature)
    end
    return WW
end
