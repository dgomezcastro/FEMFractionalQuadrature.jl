...
@testset "Test kernel convolution with CUDA" begin
    L = b - a
    if CUDA.functional()
        N = 10
        h = L / (N - 1)
        @testset "N = $N, h = $h" begin
            ...
            K = FEMFractionalQuadrature.KernelFFT2D(W_matrix, (length(X), length(Y)); use_cuda=true)
            ...
            φ_convolved = Array(φ_convolved)
            @testset "Kernel Convolution correctness" begin
                for i in 1:size(φ_matrix, 1)
                    for j in 1:size(φ_matrix, 2)
                        @test conv[i, j] ≈ φ_convolved[i, j] rtol = 1e-2
                    end
                end
            end
        end
